...
TIZEN_VM__2_2_1_armv7l_PREFIX_URL =					\
	http://download.tizen.org/releases/2.2.1/tizen-2.2_20131107.3
TIZEN_VM__2_2_1_armv7l_IMAGES_URL =				\
	$(TIZEN_VM__2_2_1_armv7l_PREFIX_URL)/images/RD-PQ

TIZEN_VM__2_2_1_armv7l_TARBALL = tizen-2.2_20131107.3_RD-PQ.tar.gz

TIZEN_VM__2_2_1_armv7l_PATCHES = $(wildcard protos/$(PROTO)/patches/*.patch)

include kernels/3.2.0-4-vexpress/rules.mk

$(PROTO)-kernel: $(DOWNLOADS)/3.2.0-4-vexpress/stamp

$(DOWNLOADS)/$(PROTO)/$(TIZEN_VM__2_2_1_armv7l_TARBALL):			\
		$(DOWNLOADS)/$(PROTO)/$(TIZEN_VM__2_2_1_armv7l_TARBALL).stamp
	touch $@

$(DOWNLOADS)/$(PROTO)/$(TIZEN_VM__2_2_1_armv7l_TARBALL).stamp:
	@mkdir -p $(dir $@) $(TMP)/$(PROTO)
	wget -O $(TMP)/$(PROTO)/$(TIZEN_VM__2_2_1_armv7l_TARBALL) \
		$(TIZEN_VM__2_2_1_armv7l_IMAGES_URL)/$(TIZEN_VM__2_2_1_armv7l_TARBALL)
	mv $(TMP)/$(PROTO)/$(TIZEN_VM__2_2_1_armv7l_TARBALL) $(dir $@)
	touch $@
...
$(TMP)/$(PROTO)/patched.stamp:			\
		$(TMP)/$(PROTO)/orig.stamp	\
		$(TIZEN_VM__2_2_1_armv7l_PATCHES)
	@mkdir -p $(dir $@)
	@rm -rf $(dir $@)patchwork
